...
## Re-executability of reconstructions (`reconstructions.py`)

The prior probes above measure *false accepts on real binaries*. This measures the other axis
the decompilation field reports — **executable correctness of a reconstruction**, the ExeBench /
LLM4Decompile re-executability metric — on a corpus anyone can read
(`benchmarks/corpus/reconstructions.jsonl`, regenerated by `build_reconstructions.py`).

Each of 12 functions (arithmetic, bitwise, control-flow, field-extract, rounding, predicate) has
a reference implementation and two reconstructions: one faithful, one with a plausible
decompilation mistake — a swapped operator, min vs max, the wrong shift amount, a rounding bug
that only shows when both operands are odd. `functions_equiv` compiles/interprets both, runs them
over shared inputs and compares: a faithful rebuild must VERIFY, a wrong one must be REFUTED with
a witness input, and **no wrong rebuild may be accepted.**

| lang   | candidates | re-executable | refuted | false accepts | missed |
|--------|-----------:|--------------:|--------:|--------------:|-------:|
| python | 24         | 12            | 12      | **0**         | 0      |
| c      | 24         | 12            | 12      | **0**         | 0      |

Python runs on every CI platform (no toolchain); C runs where a compiler is present (the
decompilation-relevant case). Both are gated on every push — a single wrong reconstruction
accepted as re-executable fails the build. Point the same runner at the published LLM4Decompile /
ExeBench datasets (the record format matches) for their numbers; the corpus here is the small,
readable, reproducible proof that the gate itself works.
