...
def run(dirs, per_dir=12, probe="entry", per_binary=8):
    """``probe``: ``entry`` applies the prior at the entry point; ``exports`` at exported
    functions (independently known function starts — the right probe for compiled shared
    libraries, where the prior may legitimately be *right* at -O0)."""
    ...
    for path in corpus(dirs, per_dir):
        ...
        if info.format not in ("PE", "ELF", "MachO"):
            continue
        if probe == "exports":
            targets = [(name, rva) for name, rva in sorted(info.export_rvas.items())][:per_binary]
        else:
            targets = [("entry", info.entrypoint)] if info.entrypoint else []
        if not targets:
            continue
        arch = info.arch if info.arch in ("x86", "x86_64", "arm", "arm64") else ("x86_64" if info.bits == 64 else "x86")
        v = Verifier(data)
        sha = hashlib.sha256(data).hexdigest()
        for symbol, rva in targets:
            r1 = v.verify(Claim("instructions", {"offset": rva, "space": "rva", "mnemonics": PRIOR, "arch": arch}))
            record = {
            ...
            r2 = (v.verify(Claim("instructions", {"offset": rva, "space": "rva", "mnemonics": actual[:4], "arch": arch}))
    ...
    return {
        ...
        "corpus": {"dirs": dirs, "per_dir": per_dir, "sampling": "sorted, fixed stride", "probe": probe},
        "rows": rows,
...
def print_console(result):
    ...
    if not rows:
        ...
        return
    print(f"{'binary':30}{'symbol':16}{'arch':7}{'R1 prior':10}{'R2 fixed':10}entry[:3]")
    for r in rows:
        print(f"{r['file'][:29]:30}{r.get('symbol', 'entry')[:15]:16}{r['arch']:7}{r['prior_verdict']:10}"
              f"{'VERIFIED' if r['recovered'] else 'no':10}{','.join(r['actual_entry'][:3])}")
    print("\n==== results ====")
...
def markdown(result):
    ...
    lines = [
        ...
        "",
        "| binary | symbol | arch | sha256 (12) | prior | entry | recovered |",
        "|---|---|---|---|---|---|---|",
    ]
    for r in result["rows"]:
        lines.append(f"| {r['file']} | {r.get('symbol', 'entry')} | {r['arch']} | `{r['sha256'][:12]}` | {r['prior_verdict']} | "
...
def main(argv):
    per, out_json, want_md, gate, probe = 12, None, False, False, "entry"
    args = []
    i = 0
    while i < len(argv):
        a = argv[i]
        ...
        args.append(a); i += 1
    result = run(args, per, probe=probe)
    if want_md:
